...
public class ThanhToanAdapter extends RecyclerView.Adapter<ThanhToanAdapter.ViewHolder>{
    ...
    public ThanhToanAdapter(Context context, ArrayList<ThanhToan> listThanhToan) {
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        ...
        holder.tvHopDongID.setText("Hợp đồng số "+thanhToan.getHopDongId());
        //Tiền dịch vụ
        ...
        holder.btnDoiTrangThai.setOnClickListener(v -> {
            boolean isUpdated = thanhToanDAO.updateTrangThaiThanhToan(thanhToan.getThanhToanId(), thanhToan.getTrangThaiThanhToan() == 0 ? 1 : 0);
            if (isUpdated) {
                thanhToan.setTrangThaiThanhToan(thanhToan.getTrangThaiThanhToan() == 0 ? 1 : 0);
                ...
                String noiDung = "Chủ trọ đã cập nhật trạng thái thanh toán cho hợp đồng số " + thanhToan.getHopDongId()+". Vui lòng kiểm tra lại trạng thái thanh toán !";
                thongBaoDAO.sendThongBao(khachThue, thanhToan.getHopDongId(), loaiThongBao, noiDung);
            } else {
        ...
        holder.btnXoa.setOnClickListener(v -> {
            if (thanhToan.getTrangThaiThanhToan() == 0) {
                boolean isDeleted = thanhToanDAO.deleteThanhToan(thanhToan.getThanhToanId());
                if (isDeleted) {
                    listThanhToan.remove(position);
                    notifyItemRemoved(position);
                    Toast.makeText(context, "Phiếu thanh toán đã được xóa!", Toast.LENGTH_SHORT).show();
                    int khachThue = hopDong.getUserId();
                    int loaiThongBao = 1;
                    String noiDung = "Chủ trọ đã xóa phiếu thanh toán cho hợp đồng số " + thanhToan.getHopDongId()+"!";
                    thongBaoDAO.sendThongBao(khachThue, thanhToan.getHopDongId(), loaiThongBao, noiDung);
                } else {
                    Toast.makeText(context, "Xóa phiếu thanh toán thất bại!", Toast.LENGTH_SHORT).show();
                }
            } else {
                Toast.makeText(context, "Chỉ có thể xóa phiếu chưa thanh toán!", Toast.LENGTH_SHORT).show();
            }
        });
